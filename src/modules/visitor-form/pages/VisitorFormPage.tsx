...
export default function VisitorFormPage() {
    ...
    const [trackingInfo, setTrackingInfo] = useState<QueueTracking | null>(
        null
    );
    ...
    const checkTrackingStatus = useCallback(
        async (forceRefresh: boolean = false, manageLoading: boolean = true) => {
            ...
            try {
                ...
                const data = await visitorFormApi.track(
                    uuid,
    ...
    const onSubmit = async (data: VisitorFormValues) => {
        try {
            ...
            toast.error(
                getErrorMessage(error, "Terjadi kesalahan saat mengirim formulir")
            );
        } finally {
            setIsLoading(false);
        }
    ...
    const markSKDFilled = async () => {
        ...
        try {
            ...
            toast.error(
                getErrorMessage(error, "Terjadi kesalahan saat memperbarui status SKD")
            );
        } finally {
            setIsLoading(false);
        }
